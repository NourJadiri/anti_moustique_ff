...
class BluetoothActions {
  ...
  static Future<void> scanForDevice({String manufactureID = '', String remoteID = '', Duration timeout = const Duration(seconds: 4)}) async {
    FlutterBluePlus.cancelWhenScanComplete(_scanSubscription!);

    await FlutterBluePlus.adapterState.where((val) => val == BluetoothAdapterState.on).first;

    await FlutterBluePlus.startScan(timeout: timeout, withNames: manufactureID.isNotEmpty ? [manufactureID] : []);
  ...
  static Future<List<BluetoothCharacteristic>> discoverCharacteristicsForServiceUUID(AntimoustiqueStruct antimoustique, String serviceUUID) async {
    try {
      return await BluetoothActions.discoverCharacteristics(
          antimoustique, antimoustique.device.servicesList.firstWhere((service) => service.uuid.toString() == serviceUUID));
    } catch (e) {
      print('Error discovering characteristics for service UUID $serviceUUID: $e');
  ...
  static Future<void> writeToCharacteristic(
    ...
    try {
      ...
      throw Exception('Error writing to characteristic ${characteristic.uuid} on device ${antimoustique.device.remoteId}');
    }
  }
